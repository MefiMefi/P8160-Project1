...
Inverse cumulative hazard function: $H_{0}^{-1}(t)=\left(\frac{t}{\lambda}\right)^{\frac{1}{\gamma}}$\
...
Similary, we take values of p from (0, 0.2, 0.4, 0.6, 0.8, 1). When `p = 1`, event time is generated from exponential baseline; when `p = 0`, event time is generated from Gompertz distribution. 

Finally, make event indicator variable by applying administrative censoring at t = 5.
